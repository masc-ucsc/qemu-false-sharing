...
G_NORETURN void riscv_raise_exception(CPURISCVState *env,
  ...
  trace_riscv_exception(exception, riscv_cpu_get_trap_name(exception, false),
                        env->pc);
...
void helper_csrw_i128(CPURISCVState *env, int csr, target_ulong srcl,
  ...
  RISCVException ret = riscv_csrrw_i128(
      env, csr, NULL, int128_make128(srcl, srch), UINT128_MAX, GETPC());
...
target_ulong helper_csrrw_i128(CPURISCVState *env, int csr, target_ulong srcl,
  ...
  RISCVException ret =
      riscv_csrrw_i128(env, csr, &rv, int128_make128(srcl, srch),
                       int128_make128(maskl, maskh), GETPC());
...
static void check_zicbom_access(CPURISCVState *env, target_ulong address,
   ...
   */
  ret = probe_access_flags(env, address, cbomlen, MMU_DATA_LOAD, mmu_idx, true,
                           &phost, ra);
...
void helper_sb_flush(CPURISCVState *env, target_ulong pc) {
  ...
  if (env->sb.log_interactions) {
    /* qemu_log("SB_FLUSH: pc=0x" TARGET_FMT_lx " count=%d\n", pc,
     * env->sb.count); */
  }
...
void helper_sb_write(CPURISCVState *env, target_ulong addr, target_ulong val,
  ...
  if (!env->sb.enabled) {
    ...
    case 1:
      ...
      break;
    case 2:
      cpu_stw_mmuidx_ra(env, addr, val, mmu_idx, ra);
      break;
    case 4:
  ...
  // Add to buffer
  int idx = env->sb.tail;
  env->sb.buffer[idx].addr = addr;
  env->sb.buffer[idx].data = val;
  env->sb.buffer[idx].size = size;

  env->sb.tail = (env->sb.tail + 1) % env->sb.capacity;
  env->sb.count++;

  /* Statistics Tracking (Cache Line Granularity: 64 bytes) */
  if (env->sb.stats) {
    uint64_t *key_addr = g_new(uint64_t, 1);
    *key_addr = addr & ~0x3F; // Align to 64-byte cache line

    SBStatsEntry *entry = g_hash_table_lookup(env->sb.stats, key_addr);
    if (!entry) {
      entry = g_new0(SBStatsEntry, 1);
      g_hash_table_insert(env->sb.stats, key_addr, entry);
    } else {
      g_free(key_addr); // Key already exists
    }
    entry->write_count++;
  }
...
target_ulong helper_sret(CPURISCVState *env) {
  ...
  if (!riscv_cpu_allow_16bit_insn(&env_archcpu(env)->cfg, env->priv_ver,
                                  env->misa_ext) &&
      (retpc & 0x3)) {
  ...
  if (riscv_cpu_cfg(env)->ext_ssdbltrp) {
    if (riscv_has_ext(env, RVH)) {
      target_ulong prev_vu =
          get_field(env->hstatus, HSTATUS_SPV) && prev_priv == PRV_U;
...
static void check_ret_from_m_mode(CPURISCVState *env, target_ulong retpc,
                                  target_ulong prev_priv, uintptr_t ra) {
  if (!(env->priv >= PRV_M)) {
    riscv_raise_exception(env, RISCV_EXCP_ILLEGAL_INST, ra);
  }
...
static target_ulong ssdbltrp_mxret(CPURISCVState *env, target_ulong mstatus,
  ...
  if (prev_priv == PRV_U ||
      (prev_virt && (prev_priv == PRV_S || prev_priv == PRV_U))) {
    mstatus = set_field(mstatus, MSTATUS_SDT, 0);
...
target_ulong helper_mret(CPURISCVState *env) {
  ...
  mstatus =
      set_field(mstatus, MSTATUS_MPP, riscv_has_ext(env, RVU) ? PRV_U : PRV_M);
  ...
  if (riscv_cpu_cfg(env)->ext_smctr || riscv_cpu_cfg(env)->ext_ssctr) {
    riscv_ctr_add_entry(env, env->pc, retpc, CTRDATA_TYPE_EXCEP_INT_RET, PRV_M,
                        false);
  }
...
target_ulong helper_mnret(CPURISCVState *env) {
  ...
  prev_virt = get_field(env->mnstatus, MNSTATUS_MNPV) && (prev_priv != PRV_M);
...
void helper_ctr_add_entry(CPURISCVState *env, target_ulong src,
  ...
  riscv_ctr_add_entry(env, src, dest, (enum CTRType)type, env->priv,
                      env->virt_enabled);
}
