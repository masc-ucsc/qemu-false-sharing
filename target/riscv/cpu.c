...
void cpu_set_exception_base(int vp_index, target_ulong address) {
  ...
  if (cs == NULL) {
    qemu_log_mask(LOG_GUEST_ERROR,
                  "cpu_set_exception_base: invalid vp_index: %u", vp_index);
  }
  cpu = RISCV_CPU(cs);
...
static void riscv_cpu_cfg_merge(RISCVCPUConfig *dest,
                                const RISCVCPUConfig *src) {
#define BOOL_FIELD(x) dest->x |= src->x;
#define TYPED_FIELD(type, x, default_)                                         \
  if (src->x != default_)                                                      \
    dest->x = src->x;
...
const char *riscv_cpu_get_trap_name(target_ulong cause, bool async) {
  if (async) {
    return (cause < ARRAY_SIZE(riscv_intr_names)) ? riscv_intr_names[cause]
                                                  : "(unknown)";
  } else {
    return (cause < ARRAY_SIZE(riscv_excp_names)) ? riscv_excp_names[cause]
                                                  : "(unknown)";
  }
}
...
#ifndef CONFIG_USER_ONLY
static void riscv_register_custom_csrs(RISCVCPU *cpu,
...
static void riscv_cpu_dump_state(CPUState *cs, FILE *f, int flags) {
  ...
  for (i = 0; i < ARRAY_SIZE(csr_ops); i++) {
     ...
     */
    if (riscv_csr_is_fpu(csrno) || riscv_csr_is_vpu(csrno)) {
      continue;
    }
  ...
  for (i = 0; i < 32; i++) {
    qemu_fprintf(f, " %-8s " TARGET_FMT_lx, riscv_int_regnames[i], env->gpr[i]);
    if ((i & 3) == 3) {
      qemu_fprintf(f, "\n");
    }
  }
  if (flags & CPU_DUMP_FPU) {
    ...
    for (i = 0; i < 32; i++) {
      qemu_fprintf(f, " %-8s %016" PRIx64, riscv_fpr_regnames[i], env->fpr[i]);
      if ((i & 3) == 3) {
        qemu_fprintf(f, "\n");
      }
    }
...
static void riscv_cpu_reset_hold(Object *obj, ResetType type) {
  ...
  if (env->misa_mxl > MXL_RV32) {
    ...
    if (riscv_has_ext(env, RVH)) {
      env->vsstatus = set_field(env->vsstatus, MSTATUS64_SXL, env->misa_mxl);
      env->vsstatus = set_field(env->vsstatus, MSTATUS64_UXL, env->misa_mxl);
      env->mstatus_hs =
...
static void riscv_cpu_satp_mode_finalize(RISCVCPU *cpu, Error **errp) {
  ...
  if (cpu->satp_modes.map == 0) {
    if (cpu->satp_modes.init == 0) {
       ...
       */
      for (int i = 1; i < 16; ++i) {
        if ((cpu->satp_modes.init & (1 << i)) && supported & (1 << i)) {
          for (int j = i - 1; j >= 0; --j) {
            if (supported & (1 << j)) {
  ...
  if (!rv32) {
    for (int i = satp_mode_map_max - 1; i >= 0; --i) {
      if (!(cpu->satp_modes.map & (1 << i)) &&
          (cpu->satp_modes.init & (1 << i)) && (supported & (1 << i))) {
        error_setg(errp,
...
static void riscv_cpu_init(Object *obj) {
  ...
  qdev_init_gpio_in_named(DEVICE(cpu), riscv_cpu_set_nmi, "riscv.cpu.rnmi",
                          RNMI_MAX);
#endif /* CONFIG_USER_ONLY */
...
#define MULTI_EXT_CFG_BOOL(_name, _prop, _defval)                              \
  { .name = _name, .offset = CPU_CFG_OFFSET(_prop), .enabled = _defval }
...
static RISCVCPUProfile RVA22U64 = {
    ...
    .ext_offsets = {CPU_CFG_OFFSET(ext_zicsr), CPU_CFG_OFFSET(ext_zihintpause),
...
static RISCVCPUProfile RVA23U64 = {
    ...
    .priv_spec = RISCV_PROFILE_ATTR_UNUSED,
    .satp_mode = RISCV_PROFILE_ATTR_UNUSED,
    .ext_offsets = {CPU_CFG_OFFSET(ext_zvfhmin), CPU_CFG_OFFSET(ext_zvbb),
...
RISCVCPUProfile *riscv_profiles[] = {
    &RVA22U64, &RVA22S64, &RVA23U64, &RVA23S64, NULL,
};

static RISCVCPUImpliedExtsRule RVA_IMPLIED = {
    .is_misa = true,
    .ext = RVA,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zalrsc),
                           CPU_CFG_OFFSET(ext_zaamo),
...
static RISCVCPUImpliedExtsRule RVF_IMPLIED = {
    .is_misa = true,
    .ext = RVF,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zicsr),
...
static RISCVCPUImpliedExtsRule RVM_IMPLIED = {
    .is_misa = true,
    .ext = RVM,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zmmul),
...
static RISCVCPUImpliedExtsRule RVV_IMPLIED = {
    .is_misa = true,
    .ext = RVV,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zve64d),
...
static RISCVCPUImpliedExtsRule ZCB_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zca),
...
static RISCVCPUImpliedExtsRule ZCE_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zcb), CPU_CFG_OFFSET(ext_zcmp),
...
static RISCVCPUImpliedExtsRule ZCMT_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zca), CPU_CFG_OFFSET(ext_zicsr),
...
static RISCVCPUImpliedExtsRule ZDINX_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zfinx),
...
static RISCVCPUImpliedExtsRule ZFH_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zfhmin),
...
static RISCVCPUImpliedExtsRule ZHINX_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zhinxmin),
...
static RISCVCPUImpliedExtsRule ZK_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zkn), CPU_CFG_OFFSET(ext_zkr),
...
static RISCVCPUImpliedExtsRule ZKN_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zbkb), CPU_CFG_OFFSET(ext_zbkc),
...
static RISCVCPUImpliedExtsRule ZVBB_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvkb),
...
static RISCVCPUImpliedExtsRule ZVE32F_IMPLIED = {
    ...
    .implied_misa_exts = RVF,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zve32x),
...
static RISCVCPUImpliedExtsRule ZVE64D_IMPLIED = {
    ...
    .implied_misa_exts = RVD,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zve64f),
...
static RISCVCPUImpliedExtsRule ZVE64F_IMPLIED = {
    ...
    .implied_misa_exts = RVF,
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zve32f),
...
static RISCVCPUImpliedExtsRule ZVFBFWMA_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvfbfmin),
                           CPU_CFG_OFFSET(ext_zfbfmin),
...
static RISCVCPUImpliedExtsRule ZVFH_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvfhmin),
...
static RISCVCPUImpliedExtsRule ZVKN_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvkned),
                           CPU_CFG_OFFSET(ext_zvknhb), CPU_CFG_OFFSET(ext_zvkb),
...
static RISCVCPUImpliedExtsRule ZVKNC_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvkn), CPU_CFG_OFFSET(ext_zvbc),
...
static RISCVCPUImpliedExtsRule ZVKNG_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvkn), CPU_CFG_OFFSET(ext_zvkg),
...
static RISCVCPUImpliedExtsRule ZVKNHB_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zve64x),
...
static RISCVCPUImpliedExtsRule ZVKS_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvksed),
                           CPU_CFG_OFFSET(ext_zvksh), CPU_CFG_OFFSET(ext_zvkb),
                           CPU_CFG_OFFSET(ext_zvkt),
...
static RISCVCPUImpliedExtsRule ZVKSC_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvks), CPU_CFG_OFFSET(ext_zvbc),
...
static RISCVCPUImpliedExtsRule ZVKSG_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_zvks), CPU_CFG_OFFSET(ext_zvkg),
...
static RISCVCPUImpliedExtsRule SSCFG_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_smcsrind),
...
static RISCVCPUImpliedExtsRule SUPM_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_ssnpm), CPU_CFG_OFFSET(ext_smnpm),
...
static RISCVCPUImpliedExtsRule SSPM_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_smnpm),
...
static RISCVCPUImpliedExtsRule SMCTR_IMPLIED = {
    ...
    .implied_multi_exts = {CPU_CFG_OFFSET(ext_sscsrind),
...
RISCVCPUImpliedExtsRule *riscv_misa_ext_implied_rules[] = {
    &RVA_IMPLIED, &RVD_IMPLIED, &RVF_IMPLIED, &RVM_IMPLIED, &RVV_IMPLIED, NULL};
...
RISCVCPUImpliedExtsRule *riscv_multi_ext_implied_rules[] = {
    ...
    &SSCFG_IMPLIED,  &SUPM_IMPLIED,     &SSPM_IMPLIED,
...
static const Property riscv_cpu_properties[] = {
    ...
    DEFINE_PROP_BOOL("rvv_vsetvl_x0_vill", RISCVCPU, cfg.rvv_vsetvl_x0_vill,
                     false),
...
#define DEFINE_ABSTRACT_RISCV_CPU(type_name, parent_type_name, ...)            \
  {                                                                            \
    .name = (type_name), .parent = (parent_type_name), .abstract = true,       \
    .class_data = &(const RISCVCPUDef){.priv_spec = RISCV_PROFILE_ATTR_UNUSED, \
...
#define DEFINE_RISCV_CPU(type_name, parent_type_name, ...)                     \
  {                                                                            \
    .name = (type_name), .parent = (parent_type_name),                         \
    .class_data = &(const RISCVCPUDef){.priv_spec = RISCV_PROFILE_ATTR_UNUSED, \
                                       .vext_spec = RISCV_PROFILE_ATTR_UNUSED, \
                                       .cfg.max_satp_mode = -1,                \
                                       __VA_ARGS__},                           \
...
#define DEFINE_PROFILE_CPU(type_name, parent_type_name, profile_)              \
  DEFINE_RISCV_CPU(type_name, parent_type_name, .profile = &(profile_))
 ...
 */
    ...
    DEFINE_ABSTRACT_RISCV_CPU(TYPE_RISCV_CPU_SIFIVE_E, TYPE_RISCV_VENDOR_CPU,
                              ...
                              .cfg.ext_zifencei = true, .cfg.ext_zicsr = true,
                              .cfg.pmp = true, .cfg.pmp_regions = 8),

    DEFINE_ABSTRACT_RISCV_CPU(
        TYPE_RISCV_CPU_SIFIVE_U, TYPE_RISCV_VENDOR_CPU,
        .misa_ext = RVI | RVM | RVA | RVF | RVD | RVC | RVS | RVU,
        .priv_spec = PRIV_VERSION_1_10_0,

        .cfg.max_satp_mode = VM_1_10_SV39, .cfg.ext_zifencei = true,
        .cfg.ext_zicsr = true, .cfg.mmu = true, .cfg.pmp = true,
        .cfg.pmp_regions = 8),
    ...
    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_IBEX, TYPE_RISCV_VENDOR_CPU, .misa_mxl_max = MXL_RV32,
        .misa_ext = RVI | RVM | RVC | RVU, .priv_spec = PRIV_VERSION_1_12_0,
        .cfg.max_satp_mode = VM_1_10_MBARE, .cfg.ext_zifencei = true,
        .cfg.ext_zicsr = true, .cfg.pmp = true, .cfg.ext_smepmp = true,

        .cfg.ext_zba = true, .cfg.ext_zbb = true, .cfg.ext_zbc = true,
        .cfg.ext_zbs = true),
    ...
    DEFINE_RISCV_CPU(TYPE_RISCV_CPU_SIFIVE_E34, TYPE_RISCV_CPU_SIFIVE_E,
                     .misa_mxl_max = MXL_RV32, .misa_ext = RVF, /* IMAFCU */
                     ),
    ...
    DEFINE_RISCV_CPU(TYPE_RISCV_CPU_RV32I, TYPE_RISCV_BARE_CPU,
                     .misa_mxl_max = MXL_RV32, .misa_ext = RVI),
    DEFINE_RISCV_CPU(TYPE_RISCV_CPU_RV32E, TYPE_RISCV_BARE_CPU,
                     .misa_mxl_max = MXL_RV32, .misa_ext = RVE),
#endif
    ...
    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_THEAD_C906, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64, .misa_ext = RVG | RVC | RVS | RVU,
        .priv_spec = PRIV_VERSION_1_11_0,

        .cfg.ext_zfa = true, .cfg.ext_zfh = true, .cfg.mmu = true,
        .cfg.ext_xtheadba = true, .cfg.ext_xtheadbb = true,
        .cfg.ext_xtheadbs = true, .cfg.ext_xtheadcmo = true,
        .cfg.ext_xtheadcondmov = true, .cfg.ext_xtheadfmemidx = true,
        .cfg.ext_xtheadmac = true, .cfg.ext_xtheadmemidx = true,
        .cfg.ext_xtheadmempair = true, .cfg.ext_xtheadsync = true,
        .cfg.pmp = true,
    ...
    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_TT_ASCALON, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64, .misa_ext = RVG | RVC | RVS | RVU | RVH | RVV,
        .priv_spec = PRIV_VERSION_1_13_0, .vext_spec = VEXT_VERSION_1_00_0,

        /* ISA extensions */
        .cfg.mmu = true, .cfg.vlenb = 256 >> 3, .cfg.elen = 64,
        .cfg.rvv_ma_all_1s = true, .cfg.rvv_ta_all_1s = true,
        .cfg.misa_w = true, .cfg.pmp = true, .cfg.cbom_blocksize = 64,
        .cfg.cbop_blocksize = 64, .cfg.cboz_blocksize = 64,
        .cfg.ext_zic64b = true, .cfg.ext_zicbom = true, .cfg.ext_zicbop = true,
        .cfg.ext_zicboz = true, .cfg.ext_zicntr = true, .cfg.ext_zicond = true,
        .cfg.ext_zicsr = true, .cfg.ext_zifencei = true,
        .cfg.ext_zihintntl = true, .cfg.ext_zihintpause = true,
        .cfg.ext_zihpm = true, .cfg.ext_zimop = true, .cfg.ext_zawrs = true,
        .cfg.ext_zfa = true, .cfg.ext_zfbfmin = true, .cfg.ext_zfh = true,
        .cfg.ext_zfhmin = true, .cfg.ext_zcb = true, .cfg.ext_zcmop = true,
        .cfg.ext_zba = true, .cfg.ext_zbb = true, .cfg.ext_zbs = true,
        .cfg.ext_zkt = true, .cfg.ext_zvbb = true, .cfg.ext_zvbc = true,
        .cfg.ext_zvfbfmin = true, .cfg.ext_zvfbfwma = true,
        .cfg.ext_zvfh = true, .cfg.ext_zvfhmin = true, .cfg.ext_zvkng = true,
        .cfg.ext_smaia = true, .cfg.ext_smstateen = true, .cfg.ext_ssaia = true,
        .cfg.ext_sscofpmf = true, .cfg.ext_sstc = true, .cfg.ext_svade = true,
        .cfg.ext_svinval = true, .cfg.ext_svnapot = true,
        .cfg.ext_svpbmt = true,

        .cfg.max_satp_mode = VM_1_10_SV57, ),

    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_VEYRON_V1, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64, .misa_ext = RVG | RVC | RVS | RVU | RVH,
        .priv_spec = PRIV_VERSION_1_12_0,

        /* ISA extensions */
        .cfg.mmu = true, .cfg.ext_zifencei = true, .cfg.ext_zicsr = true,
        .cfg.pmp = true, .cfg.ext_zicbom = true, .cfg.cbom_blocksize = 64,
        .cfg.cboz_blocksize = 64, .cfg.ext_zicboz = true, .cfg.ext_smaia = true,
        .cfg.ext_ssaia = true, .cfg.ext_sscofpmf = true, .cfg.ext_sstc = true,
        .cfg.ext_svinval = true, .cfg.ext_svnapot = true,
        .cfg.ext_svpbmt = true, .cfg.ext_smstateen = true, .cfg.ext_zba = true,
        .cfg.ext_zbb = true, .cfg.ext_zbc = true, .cfg.ext_zbs = true,
        .cfg.ext_XVentanaCondOps = true,

        .cfg.mvendorid = VEYRON_V1_MVENDORID, .cfg.marchid = VEYRON_V1_MARCHID,
        .cfg.mimpid = VEYRON_V1_MIMPID,

        .cfg.max_satp_mode = VM_1_10_SV48, ),

    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_XIANGSHAN_NANHU, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64, .misa_ext = RVG | RVC | RVB | RVS | RVU,
        .priv_spec = PRIV_VERSION_1_12_0,

        /* ISA extensions */
        .cfg.ext_zbc = true, .cfg.ext_zbkb = true, .cfg.ext_zbkc = true,
        .cfg.ext_zbkx = true, .cfg.ext_zknd = true, .cfg.ext_zkne = true,
        .cfg.ext_zknh = true, .cfg.ext_zksed = true, .cfg.ext_zksh = true,
    ...
    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_XIANGSHAN_KMH, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64,
         ...
         */
        .cfg.ext_zicntr = true, .cfg.ext_zihpm = true,
        .cfg.ext_zihintntl = true,
        .cfg.ext_zihintpause = true, .cfg.ext_zimop = true,
        .cfg.ext_zcmop =
            true,
        .cfg.ext_zicond = true, .cfg.ext_zawrs = true, .cfg.ext_zacas = true,
        .cfg.ext_zfh = true, .cfg.ext_zfa = true, .cfg.ext_zcb = true,
        .cfg.ext_zbc = true, .cfg.ext_zvfh = true, .cfg.ext_zkn = true,
        .cfg.ext_zks = true, .cfg.ext_zkt = true, .cfg.ext_zvbb = true,
        .cfg.ext_zvkt = true,
        /*
         * The RISC-V Instruction Set Manual: Volume II
         * Privileged Architecture
         */
        .cfg.ext_smstateen = true, .cfg.ext_smcsrind = true,
        .cfg.ext_sscsrind = true, .cfg.ext_svnapot = true,
        .cfg.ext_svpbmt = true, .cfg.ext_svinval = true, .cfg.ext_sstc = true,
        .cfg.ext_sscofpmf = true, .cfg.ext_ssdbltrp = true,
        .cfg.ext_ssnpm = true, .cfg.ext_smnpm = true, .cfg.ext_smmpm = true,
        .cfg.ext_sspm = true, .cfg.ext_supm = true,
        /* The RISC-V Advanced Interrupt Architecture */
        .cfg.ext_smaia = true, .cfg.ext_ssaia = true,
        /* RVA23 Profiles */
        .cfg.ext_zicbom = true, .cfg.ext_zicbop = true, .cfg.ext_zicboz = true,
        .cfg.ext_svade = true, .cfg.mmu = true, .cfg.pmp = true,
        .cfg.max_satp_mode = VM_1_10_SV48, ),

    /* https://mips.com/products/hardware/p8700/ */
    DEFINE_RISCV_CPU(
        TYPE_RISCV_CPU_MIPS_P8700, TYPE_RISCV_VENDOR_CPU,
        .misa_mxl_max = MXL_RV64,
        .misa_ext = RVI | RVM | RVA | RVF | RVD | RVC | RVS | RVU,
        .priv_spec = PRIV_VERSION_1_12_0, .cfg.max_satp_mode = VM_1_10_SV48,
        .cfg.ext_zifencei = true, .cfg.ext_zicsr = true, .cfg.mmu = true,
        .cfg.pmp = true, .cfg.ext_zba = true, .cfg.ext_zbb = true,
        .cfg.ext_xmipslsp = true, .cfg.ext_xmipscbop = true,
        .cfg.ext_xmipscmov = true, .cfg.marchid = 0x8000000000000201,
    ...
    DEFINE_RISCV_CPU(TYPE_RISCV_CPU_RV64I, TYPE_RISCV_BARE_CPU,
                     .misa_mxl_max = MXL_RV64, .misa_ext = RVI),
    DEFINE_RISCV_CPU(TYPE_RISCV_CPU_RV64E, TYPE_RISCV_BARE_CPU,
                     .misa_mxl_max = MXL_RV64, .misa_ext = RVE),
