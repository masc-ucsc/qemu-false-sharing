...
bool qemu_cpu_is_self(CPUState *cpu) { return thread_cpu == cpu; }
...
static void handle_arg_help(const char *arg) { usage(EXIT_SUCCESS); }
...
static void handle_arg_argv0(const char *arg) { argv0 = strdup(arg); }
...
static void handle_arg_seed(const char *arg) { seed_optarg = arg; }

static void handle_arg_gdb(const char *arg) { gdbstub = g_strdup(arg); }
...
static void handle_arg_rtsig_map(const char *arg) { rtsig_map = arg; }
...
static void handle_arg_strace(const char *arg) { enable_strace = true; }
...
static void handle_arg_trace(const char *arg) { trace_opt_parse(arg); }

#if defined(TARGET_XTENSA)
static void handle_arg_abi_call0(const char *arg) { xtensa_set_abi_call0(); }
#endif

static void handle_arg_perfmap(const char *arg) { perf_enable_perfmap(); }

static void handle_arg_jitdump(const char *arg) { perf_enable_jitdump(); }
...
static const struct qemu_argument arg_table[] = {
    ...
    {"t", "QEMU_RTSIG_MAP", true, handle_arg_rtsig_map, "tsig hsig n[,...]",
    ...
    {"one-insn-per-tb", "QEMU_ONE_INSN_PER_TB", false,
     handle_arg_one_insn_per_tb, "",
     "run with one guest instruction per emulated TB"},
    {"tb-size", "QEMU_TB_SIZE", true, handle_arg_tb_size, "size",
...
static void usage(int exitcode) {
  ...
  printf("%-*s %-*s Description\n", maxarglen + 1, "Argument", maxenvlen,
         "Env-variable");

  for (arginfo = arg_table; arginfo->handle_opt != NULL; arginfo++) {
    if (arginfo->has_arg) {
      printf("-%s %-*s %-*s %s\n", arginfo->argv,
             (int)(maxarglen - strlen(arginfo->argv) - 1), arginfo->example,
             maxenvlen, arginfo->env, arginfo->help);
    } else {
      printf("-%-*s %-*s %s\n", maxarglen, arginfo->argv, maxenvlen,
             arginfo->env, arginfo->help);
    }
  }
  ...
  printf("\n"
         ...
         interp_prefix, guest_stack_size);
  ...
  for (;;) {
    ...
    for (arginfo = arg_table; arginfo->handle_opt != NULL; arginfo++) {
      if (!strcmp(r, arginfo->argv)) {
        if (arginfo->has_arg) {
          if (optind >= argc) {
            (void)fprintf(stderr, "qemu: missing argument for option '%s'\n",
                          r);
            exit(EXIT_FAILURE);
          }
...
int main(int argc, char **argv, char **envp) {
  ...
  /* init tcg before creating CPUs */
  {
    ...
    object_property_set_int(OBJECT(accel), "tb-size", opt_tb_size,
                            &error_abort);
...
#pragma GCC diagnostic pop
  ...
  ret =
      loader_exec(execfd, exec_path, target_argv, target_environ, info, &bprm);
  if (ret != 0) {
